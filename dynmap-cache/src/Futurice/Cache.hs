...
genCachedIO
    ...
    -> IO v             -- ^ Value action
    -> IO v
genCachedIO policy logger cache ttl key action = do
    now <- getCurrentTime
    let expirationMoment = ttl `addUTCTime` now
        ...
        case r of
            ...
            Just (Leaf stamp r')
                ...
                | otherwise -> case policy of
                    ReturnOld  -> SP (Just errorExpirationMoment) (Just $ SP r' True)
    ...
    let cachedAction :: IO ()
        cachedAction = do
            ...
            r' <- atomically $ do
                ...
                case r' of
                    Left _exc -> pure ()
                    Right _x  -> insert key (Leaf expirationMoment r'') cache
                pure r'

            forOf_ _Left r' $ \exc ->
                runLogT "cachedIO" logger $ logAttention_ $ "Exception " <> textShow exc
